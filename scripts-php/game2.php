...
if (!$link) {               
    ...
    if((mysqli_num_rows(mysqli_query($link, "select * from gameUSP where gid = '$two' and pid = '$pid'"))) > 0){
...
if(isset($_POST["Send_Rating"]) && ($_COOKIE['rated'] >= 0)){
        ...
        mysqli_query($link,"insert into gameUSP (gid,pid,usp) values ('$two','$pid','$currRate')");
        
        if(mysqli_num_rows(mysqli_query($link, "select * from avgUSP where gid = '$two'")) > 0){   
            $prevRate = mysqli_fetch_object(mysqli_query($link, "select usp_avg from avgUSP where gid = '$two'"))->usp_avg;
            $voters = mysqli_fetch_object(mysqli_query($link, "select t_voters from avgUSP where gid = '$two'"))->t_voters;
            $currRate = intval(($prevRate * $voters + $currRate)/($voters + 1));
            $voters++;
            mysqli_query($link, "update avgUSP set t_voters = '$voters',usp_avg = '$currRate' where gid = '$two'");
        }else{
            mysqli_query($link, "insert into avgUSP (gid,usp_avg,t_voters) values ('$two','$currRate','$one')"); 
        }
