...
if (!$link) {               
    ...
    if((mysqli_num_rows(mysqli_query($link, "select * from gameUSP where gid = '$nine' and pid = '$pid'"))) > 0){
...
if(isset($_POST["Send_Rating"]) && ($_COOKIE['rated'] >= 0)){
        ...
        mysqli_query($link,"insert into gameUSP (gid,pid,usp) values ('$nine','$pid','$currRate')");
        
        if(mysqli_num_rows(mysqli_query($link, "select * from avgUSP where gid = '$nine'")) > 0){   
            $prevRate = mysqli_fetch_object(mysqli_query($link, "select usp_avg from avgUSP where gid = '$nine'"))->usp_avg;
            $voters = mysqli_fetch_object(mysqli_query($link, "select t_voters from avgUSP where gid = '$nine'"))->t_voters;
            $currRate = intval(($prevRate * $voters + $currRate)/($voters + 1));
            $voters++;
            mysqli_query($link, "update avgUSP set t_voters = '$voters',usp_avg = '$currRate' where gid = '$nine'");
        }else{
            mysqli_query($link, "insert into avgUSP (gid,usp_avg,t_voters) values ('$nine','$currRate','$one')"); 
        }
