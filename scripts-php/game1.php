...
if (!$link) {               
    ...
    $prevScr = mysqli_fetch_object(mysqli_query($link, "select score from game1 where pid = '$pid'"))->score; 
    if($score > $prevScr){
        mysqli_query($link, "update game1 set score = '$score' where pid = '$pid'");
    }
    if((mysqli_num_rows(mysqli_query($link, "select * from gameUSP where gid = '$one' and pid = '$pid'"))) > 0){
...
if(isset($_POST["Send_Rating"]) && ($_COOKIE['rated'] >= 0)){
        ...
        mysqli_query($link,"insert into gameUSP (gid,pid,usp) values ('$one','$pid','$currRate')");
        
        if(mysqli_num_rows(mysqli_query($link, "select * from avgUSP where gid = '$one'")) > 0){   
            $prevRate = mysqli_fetch_object(mysqli_query($link, "select usp_avg from avgUSP where gid = '$one'"))->usp_avg;
            $voters = mysqli_fetch_object(mysqli_query($link, "select t_voters from avgUSP where gid = '$one'"))->t_voters;
            $currRate = intval(($prevRate * $voters + $currRate)/($voters + 1));
            $voters++;
            mysqli_query($link, "update avgUSP set t_voters = '$voters',usp_avg = '$currRate' where gid = '$one'");
        }else{
            mysqli_query($link, "insert into avgUSP (gid,usp_avg,t_voters) values ('$one','$currRate','$one')"); 
        }
